...
@couriers_blu.route('/signUpCourier', methods=['GET', 'POST'])
def signUpCourier():
    data = request.get_json(force=True)

    try:
        courier = Courier(name=data['courierName'], surname=data['courierSurname'],
                          patronymic=data['courierPatronymic'], phoneNumber=data['courierPhoneNumber'],
                          # photoUrl=data['photoUrl'], 
                          email=data['courierEmail'],
                          password=generate_password_hash(data['courierPassword']),
                          courierTypeId=data['courierTypeId'],
                          # personalInformationId=data['personalInformationId'],
                          # documentsPhotoId=data['documentsPhotoId'], 
                          # courierBalance=data['courierBalance']
                          )
    ...
    return json_response(response='success', courierId=courier.id)
...
def loginCourier():
    ...
    try:
        courier = Courier.query.filter(Courier.email == data['courierEmail']).first()

        if courier and check_password_hash(courier.password, data['courierPassword']):
...
def getCourier(id):
    ...
    return json_response(courierId=courier.id, courierEmail=courier.email, 
    					 courierName=courier.name,
                         courierSurname=courier.surname, 
                         courierPatronymic=courier.patronymic,
                         courierPhoneNumber=courier.phoneNumber, 
                         courierTypeId=courier.courierTypeId) #, orders=courier.orders)
...
def editProfile(id):
    data = request.get_json(force=True)
    try:
        g.user.name = data['courierName']
        g.user.surname = data['courierSurname']
        g.user.patronymic = data['courierPatronymic']
        g.user.phoneNumber = data['courierPhoneNumber']
        g.user.courierTypeId = data['courierTypeId']
        g.user.password = generate_password_hash(data['courierPassword'])
        db.session.add(g.user)
        db.session.commit()
    except:
...
couriers_blu.route(
    '/<int:id>/orders/<int:id_order>/editStatus', methods=['POST'])
