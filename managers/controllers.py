...
@managers_blu.route('/signUpManager', methods=['GET', 'POST'])
def signUpManager():
    data = request.get_json(force=True)

    try:
        manager = Manager(name=data['managerName'], surname=data['managerSurname'],
                          patronymic=data['managerPatronymic'], 
                          phoneNumber=data['managerPhoneNumber'],
                          #photoUrl=data['photoUrl'], 
                          email=data['managerEmail'],
                          password=generate_password_hash(data['managerPassword']))
...
def loginManager():
    ...
    try:
        manager = Manager.query.filter(Manager.email == data['managerEmail']).first()

        if manager and check_password_hash(manager.password, data['managerPassword']):
...
@managers_blu.route('/<int:id>/profile', methods=['GET'])
# @login_required
def getManager(id):
    manager = Manager.query.filter(Manager.id == id).first()

    return json_response(managerId=manager.id, managerEmail=manager.email, 
...
def editProfile(id):
    data = request.get_json(force=True)
    try:
        g.user.name = data['managerName']
        g.user.surname = data['managerSurname']
        g.user.patronymic = data['managerPatronymic']
        g.user.phoneNumber = data['managerPhoneNumber']
        g.user.password = generate_password_hash(data['managerPassword'])
        db.session.add(g.user)
        db.session.commit()
    except:
