...
@orders_blu.route('/<int:id>', methods=['GET'])
# @login_required
def getOrder(id):
    ...
    #         dictionary_issue.append(dict_issue)
    ...
    return json_response(orderId = order.id, orderTypeId=order.typeId, clientId=order.clientId,
                         courierId=order.courierId,
                         orderStatusId=order.statusId,
                         orderNumberOfAddresses=order.numberOfAddresses,
                         orderInformationAboutAddresses=order.informationAboutAddresses,
                         orderDescription=order.description,
                         # orderPhoto=order.photoUrl, 
                         orderCost=order.cost)


@orders_blu.route('/free')
def getFreeOrders():
	# orders = Order.query.order_by(OrderStatus.name == 'free').all()
